...
// const BACKEND_API = 'https://demo.one2all.io/version-edrinks/api/1.1/obj/nft'
const BACKEND_API = 'https://dashboard.blockhosts.io/api/1.1/obj/nft'
...
const MarketPage = ({ navigation }) => {
  ...
  const onSearch = async () => {
    ...
    const link = `${BACKEND_API}?constraints=${body}`
  ...
  return (
    <View style={styles.root}>
        ...
        <View style={{ marginTop: 12 }}>
          ...
          {isSearchable && <View>
            ...
            <TouchableOpacity style={styles.btn} onPress={() => setIsFilterNFType(true)}>
              <Text style={styles.btnText}>Loyalty Token Category (Loyalty, Groupon etc.)</Text>
            </TouchableOpacity>

            <TouchableOpacity style={styles.btn} onPress={() => setIsMerchantType(true)}>
              <Text style={styles.btnText}>Industry Type (cafe, bar, restraint / accommodation / health etc)</Text>
            </TouchableOpacity>
          ...
          <TouchableOpacity
            ...
            style={[styles.btn, { backgroundColor: '#f9699a', borderWidth: 0, flexDirection: 'row' }]}
          >
          ...
          {allData.length > 0 && allData.map((item, index) => {
            ...
            return (<View style={{ flexDirection: 'row', backgroundColor: index !== 0 ? '#ededed' : '#ededed', padding: 10, borderRadius: 25, marginTop: 12 }}>
              <View style={{ padding: 8, backgroundColor: '#fff', borderRadius: 30, alignItems: 'center' }}>
                <Image source={{ uri: img }} style={{ width: 130, height: 130, borderRadius: 20 }} />
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                <TouchableOpacity style={{ backgroundColor: index !== 0 ? '#bebebe' : '#bebebe', padding: 8, borderRadius: 20, alignItems: 'center' }}>
                  <Text style={{ color: '#fff' }} numberOfLines={2}>{title}</Text>
                </TouchableOpacity>
                <View style={{ flexDirection: 'row', marginTop: 10, justifyContent: 'center' }}>
                  <TouchableOpacity style={{ backgroundColor: '#174b42', padding: 10, borderRadius: 12, alignItems: 'center', flex: 1 }}>
                    <Text style={{ color: '#fff', fontSize: 8 }}>Available</Text>
                    <Text style={{ color: '#fff', fontSize: 12 }}>{editions}</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={{ backgroundColor: '#f2b8cc', padding: 10, borderRadius: 12, alignItems: 'center', marginLeft: 8, flex: 1, }}>
                    <Text style={{ color: '#fff', fontSize: 8 }}>Price</Text>
                    <Text style={{ color: '#fff', fontSize: 10, textAlign: 'center' }}>{price || '0'} MATIC</Text>
                  </TouchableOpacity>
                </View>
                <View style={{ marginTop: 12, maxHeight: 40, overflow: 'hidden' }}>
                  <Text style={{ color: index !== 0 ? '#1f7868' : '#1f7868', fontSize: 10 }}>
                    {description}
                  </Text>
                </View>
              </View>
...

      {
        isFilterNFType &&
        <ModalFilter
          data={['Gift Cards', 'Vouchers', 'Coupons', 'Rewards', 'Tickets']}
...

      {
        isMerchantType &&
        <ModalFilter
          data={['Cafes, Bars & Restaurants', 'Health & Fitness', 'Hotels & Accommodation', 'Beauty', 'Fashion']}
          onClose={onCloseModal}
